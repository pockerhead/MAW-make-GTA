...
- Следующий: TASK-008 (T7) — ближний бой (кулаки, бита, комбо, нокдаун).
...
Обновлено: 2026-09-23 19:48. Закрыто задач: 8. Время работы агентов: ~17 ч.

| Кто | Вход | Выход | ≈ USD |
|---|---|---|---|
| Claude Opus 5.5 (оркестратор + сабагенты) | 0.01M без кэша, 638M чтение кэша, 13.1M запись в кэш | 1.20M | $217–257 |
| Codex `gpt-6-sol` | 142.1M вход (из них 139.1M из кэша) | 0.34M | $37 |
...
ЛКМ захватывает (или стреляет, если в руках ствол). Оружие: `2` пистолет, `3` SMG, `4` дробовик, `1` убрать,
...
QA запущенного билда: `python tools/qa/scenarios/t1.py --out <dir>`, `t2.py`, `t3.py`, `t4.py`, `t5.py`, `t6.py` (фича `dev`, BRP на порту 15702).
