...
- Q1 HUD / death-screen numbers in `strings.ron` (no new hud.ron outside GDD §12).
- Q2 no death animation in T5.
- Q3 medkit and armour pickups both near the hospital.
- Q4 debug damage on F5 behind the `debug` feature.
Also accepted: Inter 4.1 (SIL OFL 1.1) as the Cyrillic font via the asset manifest (manifest schema extended beyond Kenney); player spawn and city mesh build move to `OnTransition{Loading -> Playing}` so returning from Wasted spawns neither twice.
