...
- Q1 heat below the first star decays by the 1st-star rule (GTA-like forgiveness for minor crimes); accepted default.
- Q2 `shooting_merge_seconds` 5 s and `incident_memory_seconds` 60 s stay as RON knobs, tuned after the owner run.
- Q3 repeat calls about one corpse stay visible (heat counted once); a civilian's memory of past calls is out of scope.
