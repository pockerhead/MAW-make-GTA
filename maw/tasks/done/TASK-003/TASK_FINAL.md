...
- Q1 city edge: **B** — invisible walls along the edge of the ground margin (4 static colliders, wall height in `city.ron`). Falling off the world in a sandbox is a bad default; the cost is ~10 lines. Keep the 100 m ground margin.
- Q2 v1 simplifications of GDD §2.2 (core/outskirts grid step instead of the district table, Voronoi districts after the grid, no density noise, alleys as grid edges, one conflict zone per intersection, park trees in T3): **accepted** — none changes the `CityLayout` interface.
- Q3 Latin loading-screen string until a Cyrillic font arrives with `strings.ron` in T5: **accepted**.
