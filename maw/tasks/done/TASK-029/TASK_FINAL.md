...
- Q1 OS: Linux only for test CI (MAW stages already run everything on Windows locally; TASK-028 release CI will build Windows). Revisit if a Windows-only failure ever escapes.
- Q2 Linux physics divergence: if any gate is red only on Linux, do NOT loosen tolerances in this task — record it and open a follow-up task with the evidence; the CI job for that binary may be temporarily marked with the failing test list in the summary, never silently skipped.
