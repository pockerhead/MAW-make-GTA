...
- Q1 a hit on a gang member while the player is outside the territory: **always provokes** (self-defence; "nothing happens" reads as a bug). Keep `hit_outside_the_territory_still_provokes`.
- Q2 warning: **the gang member draws, aims and walks toward the player** (readable threat before the fight).
- Q3 `GangHeat` keeps decaying while the player is dead/Wasted (GTA-like forgiveness; no freeze).
- Q4 weapons and gang colours: the planner's defaults, all in `assets/gang/gangs.ron`.
