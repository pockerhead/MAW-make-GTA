...
- Format only the files you created or edited (`rustfmt --edition 2024 <file>` — match the edition in `Cargo.toml`); never `cargo fmt` on the whole workspace. `rustfmt` on a crate root / `mod.rs` also formats every child module it declares: after formatting, run `git diff --stat` and revert any file you did not edit (TASK-004).
